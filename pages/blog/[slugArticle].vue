...
<template>
  ...
  <article v-else v-for="article in articles" :key="article.article_slug"
    class="flex flex-col gap-8 justify-start sm:px-10 md:px-18 items-start pb-12" itemscope
    ...
    <div class="flex flex-col gap-6 w-full ">
      ...
      <div class="">
        <img itemprop="image" class="md:rounded-xl rounded-xl object-cover w-full"
          v-bind="{ 'src': `${article.article_cover}`, 'alt': `${article.article_title} | waro colombia` }">
      </div>
      <div class=" flex  flex-col sm:flex-row gap-4 ">
        <div class=" flex gap-4">
          <img itemprop="image" class="rounded-lg object-cover w-20"
            v-bind="{ 'src': `${article.creator_profile_pic}`, 'alt': `${article.creator_full_name} | waro colombia` }">
          <div class="flex flex-col">
            <div itemprop="author">
              <p itemprop="name" class="text-normal sm:text-lg font-bold">
                {{ article.creator_full_name }}
              </p>
            </div>
            <p class="text-sm sm:text-normal w-3/4">
        ...
        <div class="w-auto">
          <CommonTheShareBox v-bind:slug="route.path">
          </CommonTheShareBox>
        </div>
      </div>
    </div>
    ...
    <CommonTheFeedBackBox v-bind:id="route.path"></CommonTheFeedBackBox>
    ...
    <Head>
      ...
      <Meta property="og:image" :content="`${article.article_cover}`" />
      ...
      <Meta name="twitter:image" :content="`${article.article_cover}`" />
    </Head>
