...
CREATE TABLE public.sessions (
    ...
    ip_address inet,
    user_agent text,
    login_method character varying(50) DEFAULT 'magic_link'::character varying,
    is_active boolean DEFAULT true,
...
CREATE TABLE public.templates (
    ...
    name character varying(255) DEFAULT 'Unnamed Template'::character varying NOT NULL,
    pair_id uuid
);
...
CREATE INDEX idx_email_clicks_send_id ON public.email_clicks USING btree (email_send_id);
CREATE INDEX idx_email_opens_send_id ON public.email_opens USING btree (email_send_id);
CREATE INDEX idx_email_sends_status ON public.email_sends USING btree (status);
CREATE INDEX idx_email_sends_message_id ON public.email_sends USING btree (message_id);
CREATE INDEX idx_email_sends_email ON public.email_sends USING btree (email);
...
CREATE INDEX idx_magic_tokens_used ON public.magic_tokens USING btree (used);
CREATE INDEX idx_magic_tokens_user_id_created ON public.magic_tokens USING btree (user_id, created_at);
CREATE INDEX idx_magic_tokens_created_at ON public.magic_tokens USING btree (created_at);
...
CREATE INDEX idx_templates_active_version_id ON public.templates USING btree (active_version_id);
...
ALTER TABLE public.email_clicks
    ADD CONSTRAINT fk_email_clicks_send FOREIGN KEY (email_send_id) REFERENCES email_sends(id) ON DELETE CASCADE;
...
ALTER TABLE public.email_opens
    ADD CONSTRAINT fk_email_opens_send FOREIGN KEY (email_send_id) REFERENCES email_sends(id) ON DELETE CASCADE;
ALTER TABLE public.email_sends
    ADD CONSTRAINT email_sends_pkey PRIMARY KEY (id);
ALTER TABLE public.email_sends
    ADD CONSTRAINT fk_email_sends_campaign FOREIGN KEY (campaign_id) REFERENCES campaign(id) ON DELETE CASCADE;
ALTER TABLE public.email_sends
    ADD CONSTRAINT fk_email_sends_lead FOREIGN KEY (lead_id) REFERENCES leads(id) ON DELETE CASCADE;
...
ALTER TABLE public.templates
    ADD CONSTRAINT fk_templates_active_version_id FOREIGN KEY (active_version_id) REFERENCES template_versions(id) ON DELETE SET NULL;
ALTER TABLE public.templates
...
CREATE OR REPLACE FUNCTION public.create_campaign_from_pair(p_profile_id uuid, p_campaign_name character varying, p_pair_id uuid)
 RETURNS json
 LANGUAGE plpgsql
AS $function$
DECLARE
    ...
    v_base_slug text;
    v_counter integer := 1;
    v_final_campaign_json json;
BEGIN
    -- 1. Verificar que el par de templates es válido (contiene exactamente un email y un landing).
    -- Si no es válido, la subconsulta no devolverá filas y la condición `NOT FOUND` lanzará un error.
    IF NOT EXISTS (
        SELECT 1
        FROM templates t
        WHERE t.pair_id = p_pair_id
        GROUP BY t.pair_id
        HAVING
            COUNT(*) = 2 AND
            COUNT(*) FILTER (WHERE t.template_type = 'email') = 1 AND
            COUNT(*) FILTER (WHERE t.template_type = 'landing') = 1
    ) THEN
        RAISE EXCEPTION 'El par de templates no es válido o no contiene un email y una landing page. pair_id: %', p_pair_id;
    END IF;

    -- 2. Generar un slug único para la campaña.
    -- Normaliza el nombre de la campaña para crear un slug base.
    v_base_slug := lower(
        regexp_replace(
            regexp_replace(
                regexp_replace(
                    normalize(p_campaign_name, NFD),
                    '[\u0300-\u036f]', '', 'g'
                ), '[^a-z0-9\s-]', '', 'g'
            ), '\s+', '-', 'g'
        )
    ...
    -- Bucle para asegurar que el slug sea único, añadiendo un contador si es necesario.
    LOOP
        IF NOT EXISTS (SELECT 1 FROM campaign WHERE slug = v_slug) THEN
            EXIT; -- El slug es único, salimos del bucle.
        END IF;
        v_slug := v_base_slug || '-' || v_counter;
        v_counter := v_counter + 1;
    END LOOP;

    -- 3. Crear la campaña en la tabla 'campaign'.
    INSERT INTO campaign (profile_id, name, status, slug)
    VALUES (p_profile_id, p_campaign_name, 'active', v_slug)
    RETURNING id INTO v_campaign_id;

    -- 4. Asociar las versiones activas de los templates del par con la nueva campaña.
    -- Esto se hace en una sola consulta INSERT-SELECT para mayor eficiencia.
    INSERT INTO campaign_template_versions (campaign_id, template_version_id)
    SELECT v_campaign_id, t.active_version_id
    FROM templates t
    WHERE t.pair_id = p_pair_id AND t.active_version_id IS NOT NULL;

    -- 5. Obtener y construir el objeto JSON completo de la campaña para retornarlo.
    -- Esta consulta es similar a la última del endpoint original.
    SELECT
        json_build_object(
            'id', c.id,
    ...
    INTO v_final_campaign_json
    FROM campaign c
    JOIN campaign_template_versions ctv ON c.id = ctv.campaign_id
...
CREATE OR REPLACE FUNCTION public.create_template_pair(p_template_name text, p_description text, p_sender_email text, p_subject_template text, p_email_content text, p_landing_title text, p_landing_description text, p_landing_image_url text)
 RETURNS TABLE(pair_id uuid, email_template_id uuid, landing_template_id uuid, email_template_name text, landing_template_name text, status_message text)
 LANGUAGE plpgsql
AS $function$
  DECLARE
    v_pair_id uuid;
    ...
    v_email_template_version_id uuid;
    v_landing_template_version_id uuid;
    v_email_template_name text;
    v_landing_template_name text;
    v_landing_content json;
  BEGIN
    -- Generar UUID para el par
    v_pair_id := gen_random_uuid();

    -- Generar nombres de templates
    v_email_template_name := p_template_name || ' - Email';
    v_landing_template_name := p_template_name || ' - Landing';

    -- Crear JSON para landing
    v_landing_content := json_build_object(
      'title', p_landing_title,
      'description', p_landing_description,
      'image', json_build_object(
        'type', 'image',
        'content', p_landing_image_url
      )
    );

    -- 1. Crear template de email
    INSERT INTO templates (template_name, template_type, description, subject_template, sender_email, pair_id)
    VALUES (v_email_template_name, 'email', p_description, p_subject_template, p_sender_email, v_pair_id)
    RETURNING id INTO v_email_template_id;

    -- 2. Crear versión del template de email
    INSERT INTO template_versions (template_id, content, version_number)
    VALUES (v_email_template_id, p_email_content, 1)
    RETURNING id INTO v_email_template_version_id;

    -- 3. Actualizar active_version_id del template de email
    UPDATE templates
    SET active_version_id = v_email_template_version_id
    WHERE id = v_email_template_id;

    -- 4. Crear template de landing
    INSERT INTO templates (template_name, template_type, description, sender_email, pair_id)
    VALUES (v_landing_template_name, 'landing', p_description, p_sender_email, v_pair_id)
    RETURNING id INTO v_landing_template_id;

    -- 5. Crear versión del template de landing
    INSERT INTO template_versions (template_id, content, version_number)
    VALUES (v_landing_template_id, v_landing_content::text, 1)
    RETURNING id INTO v_landing_template_version_id;

    -- 6. Actualizar active_version_id del template de landing
    UPDATE templates
    SET active_version_id = v_landing_template_version_id
    WHERE id = v_landing_template_id;

    -- Retornar resultados
    RETURN QUERY SELECT
      v_pair_id,
      v_email_template_id,
      v_landing_template_id,
      v_email_template_name,
      v_landing_template_name,
      'Templates creados exitosamente'::text;
...
CREATE OR REPLACE FUNCTION public.update_campaign_templates(p_campaign_id uuid, p_email_content text, p_landing_content text, p_subject_template text, p_email_template_name text, p_landing_template_name text)
 RETURNS TABLE(updated_email_version_id uuid, updated_landing_version_id uuid)
 LANGUAGE plpgsql
AS $function$
DECLARE
    v_email_template_id uuid;
    v_landing_template_id uuid;
    v_new_email_version_id uuid;
    v_new_landing_version_id uuid;
    v_old_email_version_id uuid;
    v_old_landing_version_id uuid;
BEGIN
    -- === 1. Encontrar y actualizar la plantilla de EMAIL ===

    -- Encuentra el template_id y la versión_id actual del email asociado a esta campaña
    SELECT t.id, ctv.template_version_id INTO v_email_template_id, v_old_email_version_id
    FROM public.templates AS t
    JOIN public.template_versions AS tv ON t.id = tv.template_id
    JOIN public.campaign_template_versions AS ctv ON tv.id = ctv.template_version_id
    WHERE ctv.campaign_id = p_campaign_id AND t.template_type = 'email'
    LIMIT 1;

    IF v_email_template_id IS NULL THEN
        RAISE EXCEPTION 'No se encontró la plantilla de email para la campaña ID %', p_campaign_id;
    END IF;

    -- Crea una nueva versión para la plantilla de email
    INSERT INTO public.template_versions (template_id, version_number, content)
    VALUES (
        v_email_template_id,
        (SELECT COALESCE(MAX(version_number), 0) + 1 FROM public.template_versions WHERE template_id = v_email_template_id),
        p_email_content
    )
    RETURNING id INTO v_new_email_version_id;

    -- Actualiza el registro principal de la plantilla (nombre, asunto, etc.)
    UPDATE public.templates
    SET
        name = p_email_template_name,
        template_name = p_email_template_name,
        subject_template = p_subject_template,
        active_version_id = v_new_email_version_id
    WHERE id = v_email_template_id;

    -- === 2. Encontrar y actualizar la plantilla de LANDING ===

    -- Encuentra el template_id y la versión_id actual de la landing
    SELECT t.id, ctv.template_version_id INTO v_landing_template_id, v_old_landing_version_id
    FROM public.templates AS t
    JOIN public.template_versions AS tv ON t.id = tv.template_id
    JOIN public.campaign_template_versions AS ctv ON tv.id = ctv.template_version_id
    WHERE ctv.campaign_id = p_campaign_id AND t.template_type = 'landing'
    LIMIT 1;
    
    IF v_landing_template_id IS NULL THEN
        RAISE EXCEPTION 'No se encontró la plantilla de landing page para la campaña ID %', p_campaign_id;
    END IF;

    -- Crea una nueva versión para la plantilla de landing
    INSERT INTO public.template_versions (template_id, version_number, content)
    VALUES (
        v_landing_template_id,
        (SELECT COALESCE(MAX(version_number), 0) + 1 FROM public.template_versions WHERE template_id = v_landing_template_id),
        p_landing_content
    )
    RETURNING id INTO v_new_landing_version_id;

    -- Actualiza el registro principal de la plantilla
    UPDATE public.templates
    SET
        name = p_landing_template_name,
        template_name = p_landing_template_name,
        active_version_id = v_new_landing_version_id
    WHERE id = v_landing_template_id;
    
    -- === 3. Actualizar la tabla de asociación para que apunte a las NUEVAS versiones ===
    
    UPDATE public.campaign_template_versions
    SET template_version_id = v_new_email_version_id
    WHERE campaign_id = p_campaign_id AND template_version_id = v_old_email_version_id;

    UPDATE public.campaign_template_versions
    SET template_version_id = v_new_landing_version_id
    WHERE campaign_id = p_campaign_id AND template_version_id = v_old_landing_version_id;

    RETURN QUERY SELECT v_new_email_version_id, v_new_landing_version_id;

EXCEPTION
    WHEN OTHERS THEN
        RAISE NOTICE 'Error in update_campaign_templates: %', SQLERRM;
        RAISE;
END;
$function$
;
...
CREATE OR REPLACE FUNCTION public.update_template_pairs(p_pair_id uuid, p_email_content text, p_landing_content text, p_subject_template text, p_email_name character varying, p_landing_name character varying)
 RETURNS TABLE(success boolean, message text, updated_templates integer, email_template_id uuid, landing_template_id uuid)
 LANGUAGE plpgsql
AS $function$
  DECLARE
      email_template_id_var UUID;
      landing_template_id_var UUID;
      email_version_id UUID;
      landing_version_id UUID;
      updated_count INTEGER := 0;
  BEGIN
      -- Find the email and landing template IDs for this pair
      SELECT id INTO email_template_id_var
      FROM templates
      WHERE pair_id = p_pair_id AND template_type = 'email';

      SELECT id INTO landing_template_id_var
      FROM templates
      WHERE pair_id = p_pair_id AND template_type = 'landing';

      -- Check if both templates exist
      IF email_template_id_var IS NULL THEN
          RETURN QUERY SELECT false, format('No se encontró la plantilla de email para la campaña ID %s', p_pair_id), 0, NULL::UUID,
  NULL::UUID;
          RETURN;
      END IF;

      IF landing_template_id_var IS NULL THEN
          RETURN QUERY SELECT false, format('No se encontró la plantilla de landing para la campaña ID %s', p_pair_id), 0, NULL::UUID,
  NULL::UUID;
          RETURN;
      END IF;

      -- Update email template (using actual columns that exist)
      UPDATE templates
      SET
          template_name = p_email_name,
          name = p_email_name,
          subject_template = p_subject_template
      WHERE id = email_template_id_var;

      updated_count := updated_count + 1;

      -- Update landing template (using actual columns that exist)
      UPDATE templates
      SET
          template_name = p_landing_name,
          name = p_landing_name
      WHERE id = landing_template_id_var;

      updated_count := updated_count + 1;

      -- Create new versions in template_versions (using actual columns)
      INSERT INTO template_versions (
          ...
          gen_random_uuid(),
          email_template_id_var,
          (SELECT COALESCE(MAX(version_number), 0) + 1 FROM template_versions WHERE template_id = email_template_id_var),
          p_email_content,
          CURRENT_TIMESTAMP
      ) RETURNING id INTO email_version_id;
      ...
      INSERT INTO template_versions (
          ...
          gen_random_uuid(),
          landing_template_id_var,
          (SELECT COALESCE(MAX(version_number), 0) + 1 FROM template_versions WHERE template_id = landing_template_id_var),
          p_landing_content,
          CURRENT_TIMESTAMP
      ) RETURNING id INTO landing_version_id;

      -- Update active version IDs (this column exists after migrations)
      UPDATE templates
      SET active_version_id = email_version_id
      WHERE id = email_template_id_var;

      UPDATE templates
      SET active_version_id = landing_version_id
      WHERE id = landing_template_id_var;

      RETURN QUERY SELECT true, 'Templates actualizados correctamente'::TEXT, updated_count, email_template_id_var,
  landing_template_id_var;

  EXCEPTION
      WHEN OTHERS THEN
          RETURN QUERY SELECT false, format('Error al actualizar templates: %s', SQLERRM), 0, NULL::UUID, NULL::UUID;
  END;
  $function$
;
